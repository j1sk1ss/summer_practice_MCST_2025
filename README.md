# Задания
## Навигация
- [Параллельная сортировка массива с использованием pthreads](#1-параллельная-сортировка-массива-с-использованием-pthreads)
- [Межпроцессный чат через именованные каналы и сокеты](#2-межпроцессный-чат-через-именованные-каналы-или-сокеты)
- [Сортировка строк из файла с удалением пустых строк](#3-сортировка-строк-из-файла-с-удалением-пустых-строк)
- [Парсер аргументов командной строки с использованием getopt и getopt_long](#4-парсер-аргументов-командной-строки-с-использованием-getopt-и-getopt_long)
- [Демонстрация работы с ошибками системных вызовов](#5-демонстрация-работы-с-ошибками-системных-вызовов)
- [Получение информации о файлах с помощью stat](#6-получение-информации-о-файлах-с-помощью-stat)
- [Чтение и копирование файла в дочернем и родительском процессах](#7-чтение-и-копирование-файла-в-дочернем-и-родительском-процессах)
- [Умножение матриц с обработкой SIGINT](#8-умножение-матриц-с-обработкой-sigint)
- [Максимальная возрастающая подпоследовательность с максимальной суммой](#9-максимальная-возрастающая-подпоследовательность-с-максимальной-суммой)
- [Парсер логических выражений](#10-парсер-логических-выражений)
...
### Описание решения
В одном исполнительном файле одновременно реализовано два варианта комуникации процессов. Для удобства запуска и наглядной демонстрации, главный процесс делает `fork()`, а далее реализует алгоритм взаимодействия. </br>
...
### Описание решения
Данный исполнительный файл реализует сортировку строк файла по разным паттернам. Перед сортировкой удаляются все пустые строки и строки с невидимыми символами. </br>
...
Основная идея, что в токене хранится не только сама строка, но и её временная реализация с числовым значением, по которому будет происходить сортировка. Например при команде `--tlower`, а после `--bylen`, во временную строку запишется строка в нижнем регистре, а во временном числовом значении будет лежать длина строки (длина именно временного представления, если оно есть).</br>
...
### Описание решения
Реализованное решение представляет из себя два файла. Это `parser.c` и `parser_old.c`. Дело в том, что изначальное я не достаточно верно прочитал поставленную задачу, и проигнорировал встроенные инструменты `getopt` и `getopt_long`, результатом чего является файл `parser_old.c`.
...
### Описание решения
Реализованная программа демонстрирует сразу три способа вывода ошибки. Для удобства, также можно выбирать какая ошибка будет использоваться. Делается это с помощью комбинации флагов (или одного флага) `--errlist`, `--perror` и `--errno`. Если же флагов нет, будет выведенно сразу три варианта.
...
### Описание решения
Данная программа скопирует файл по пути, который будет передан как первый аргумент. Нюанс работы программы заключается в том, что файлы, по стандарту, будут сохранятся в директории вызова исполнительного файла. Для изменения следует использовать опцию `-o`, в которую следует передать базовый путь до директории сохранения.
...
Вывод ответа происходит в консоль. Так же для удобства был сгенерирован файл последовательности `something.txt`.
...
### Описание решения
За основу был взят алгоритм моего компилятора `CordellCompiler.EXMPL`, а именно алгоритм токенизации, разметки, семантического анализа и генерации бинарного дерева. </br>
...
- Набор токенов проходит процедуру разметки. Выражения `and`, `or`, `not` помечаются соответственными токенами, остальные же буквенные выражения становятся переменными (Переменным одновременно выдаются индекс).
- Размеченные токены проходят процесс семантического анализа, для первичного отсеивания ошибок неверной скобочной последовательности, и неверного синтаксиса по типу:
    - Подряд идущие логические операнды.
    - Подряд идущие переменные.
    - Идущая переменная перед открытой скобкой.
- После семантического анализа проходит генерация бинарного дерева. Саму структуру древа можно увидеть при добавлении параметра `--st`.
- Ответом считается сгенерированная таблица истинности для данного выражения.

------------------------------
# Заключение
Выполнение заданий оконченно 22.04.2025, в 9:00 МСК. </br>
Обновлён главный README.md файл для исправления ряда ошибок. </br>
Задачи протестированны на MacOSX 10.15, Linux Alpine, Linux Debian (ядро 6.1.0-33-amd64). </br>
